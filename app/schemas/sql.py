...
class DatabaseInfo(BaseModel):
    ...
    relationships: List[Relationship] = Field(default_factory=list, description="List of relationships between tables")
...
class SQLGenerationOutput(BaseModel):
    ...
    sql_query: Optional[str] = Field(None, description="The generated SQL query (alias for backward compatibility)")
    ...
    error_message: Optional[str] = Field(None, description="Error message if SQL generation failed")
    
    @validator('sql_query', always=True)
    def sync_sql_query(cls, v, values):
        """Ensure sql_query is always the same as sql."""
        if 'sql' in values:
            return values['sql']
        return v 