...
class QueryResult(BaseModel):
    ...
    column_names: Optional[List[str]] = Field(
        None, description="Column names in result"
    )
    columns: Optional[List[Dict[str, Any]]] = Field(
        None, description="Column information including name, type, and display size"
    )
    ...
    warnings: List[str] = Field(
        default_factory=list, description="Warnings during execution"
    )
    has_more_rows: Optional[bool] = Field(
        None, description="Whether there are more rows than returned"
    )
...
class ExplanationType(str, Enum):
    ...
    CONVERSATIONAL = "CONVERSATIONAL"  # Natural conversation response
...
class UserExplanation(BaseModel):
    ...
    type: ExplanationType = Field(
        default=ExplanationType.SIMPLIFIED, description="Type of explanation"
    )
    referenced_concepts: List[str] = Field(
        default_factory=list, description="SQL concepts referenced"
    )
...
class UserResponse(BaseModel):
    ...
    sql_generation: Optional[SQLGenerationOutput] = Field(
        None, description="Generated SQL"
    )
    query_result: Optional[QueryResult] = Field(
        None, description="Query execution result"
    )
    explanation: Optional[UserExplanation] = Field(
        None, description="User-friendly explanation"
    )
