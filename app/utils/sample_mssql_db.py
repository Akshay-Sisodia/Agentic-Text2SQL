"""Utility to set up a sample MSSQL (SQL Server) database for testing."""
...
def create_sample_mssql_database():
    """Create a sample MSSQL database for testing."""
    # Make sure we're using MSSQL
    if settings.DB_TYPE != 'mssql':
        logger.warning("This utility is only for MSSQL databases. Skipping sample database creation.")
        return
    
    # Create engine and metadata
    connection_str = f"mssql+pyodbc://{settings.DB_USER}:{settings.DB_PASSWORD}@{settings.DB_HOST}:{settings.DB_PORT}/{settings.DB_NAME}?driver=ODBC+Driver+17+for+SQL+Server"
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            logger.info(f"Successfully connected to MSSQL database: {settings.DB_NAME}")
        ...
        # Check if tables exist and drop them before creating
        with engine.connect() as conn:
        ...
        logger.info("Created sample tables in MSSQL database")
        ...
        logger.info("Inserted sample data into MSSQL database")
        
    except Exception as e:
        logger.error(f"Error creating sample MSSQL database: {str(e)}")
        raise